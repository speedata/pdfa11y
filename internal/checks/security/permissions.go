...
// "extract for accessibility" permission. PDF/UA-1 §7.16 forbids
...
// directly.
//
// Spec gating: PDF/UA-1 only. The /P bit-10 requirement lives in
// ISO 14289-1 §7.16 (Matterhorn 26-001/26-002). ISO 14289-2 has no
// security/encryption clause at all (it defers to ISO 32000-2), so
// this check would have no normative basis on a UA-2 document.
...
func (Permissions) Spec() engine.Spec         { return engine.SpecPDFUA1 }
func (Permissions) WCAG() []string            { return []string{"1.3.1"} }
func (Permissions) Description() string {
	return "PDF/UA-1 §7.16 requires the document's security handler to grant assistive-technology extraction. In the standard PDF security handler this is /P bit 10 (mask 512); a permission set that clears that bit makes the PDF inaccessible by construction, even if every other PDF/UA rule is satisfied."
}
...
func (c Permissions) Run(doc model.Document) []engine.Finding {
	...
	if !enc.AllowAccessibility {
		return []engine.Finding{{
			...
			Hint:     "Update the security handler so /P keeps bit 10 set (mask 512). PDF/UA-1 §7.16 makes this bit mandatory for accessible documents.",
		}}
	}
