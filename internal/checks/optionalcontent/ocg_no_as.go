...
//
// ISO 14289-2 §8.7 (PDF/UA-2): "The AS key shall not appear in any
// optional content configuration dictionary." The /AS key declares
// automatic state-change rules that adjust OCG visibility based on
// usage information (Print, Export, View). PDF/UA-2 forbids it so
// optional-content visibility cannot silently flip behind AT's
// back during reading.
//
...
func (OCGNoAS) Spec() engine.Spec         { return engine.SpecPDFUA2 }
func (OCGNoAS) WCAG() []string            { return []string{"1.3.1"} }
func (OCGNoAS) Description() string {
	return "ISO 14289-2 §8.7 forbids the /AS key in any Optional Content configuration dictionary. /AS declares automatic state-change rules that adjust layer visibility based on usage information (Print, Export, View). PDF/UA-2 disallows it so optional-content visibility cannot silently change behind assistive technology's back during reading."
}
...
func (c OCGNoAS) Run(doc model.Document) []engine.Finding {
	...
	return []engine.Finding{{
		CheckID:  c.ID(),
		Severity: engine.SeverityError,
		Message:  "default optional-content configuration /OCProperties/D declares /AS, which is forbidden by PDF/UA-2",
